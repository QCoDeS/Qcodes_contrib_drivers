...
class TinySASerialBackend:
    ...
    def _coerce_trace_length(
        ...
        if array.size == expected_points:
            return array
        if array.size > expected_points:
            return array[:expected_points]
        if array.size == 0:
            return np.full(expected_points, np.nan, dtype=float)
        padding = np.full(expected_points - array.size, array[-1], dtype=float)
        return np.concatenate((array, padding))

    @staticmethod
    def _parse_scan_column(payload: bytes, expected_points: int) -> np.ndarray:
    ...
    def scan(
        ...
        *,
        ...
        try:
            return self._parse_scan_column(payload, expected_points=points)
        finally:
            self.resume()
...
class TinySABasic(Instrument):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.add_parameter(
            ...
            vals=Enum(*self.ALLOWED_SWEEP_POINTS),
        )
        self.add_parameter(
            "frequency",
            label="Frequency",
    ...
    def _set_points(self, value: int) -> None:
        points = int(value)
        if points not in self.ALLOWED_SWEEP_POINTS:
            raise ValueError(
                "tinySA supports only these sweep point counts: "
                f"{', '.join(str(p) for p in self.ALLOWED_SWEEP_POINTS)}. "
                f"Requested: {points}."
            )
        self._points = points
        self._invalidate_trace_cache()
