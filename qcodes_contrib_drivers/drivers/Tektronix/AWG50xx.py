...
_chan_resolution_docstrings = {'5208': "12 bit resolution allows for four "
                               ...
                               '5204': "12 bit resolution allows for four "
                                       "markers, 13 bit resolution "
                                       "allows for three, etc. with 16 bit "
                                       "allowing for ZERO markers",
...
class AWG70000A(VisaInstrument):
    ...
    def __init__(self, name: str, address: str, num_channels: int,
        ...
        if self.model not in ['70001A', '70002A', '5208', '5204']:
    ...
    def jump_elem_in_seq(self,seq_number):
        """
        Force a seq to jump to element seq_number
        """      
        self.write(':JUMP:FORCe {}'.format(seq_number))
    ...
    def saveSEQXFile(self,filename: str, seqname :  str= 'main_sequence', path: str=None) -> None:
        """
        Save a seqx file from instrument memory to instrument disk.
        ...
        pathstr = 'C:{}\\{}.seqx'.format(path, filename)

        self.write('MMEMORY:SAVE:SEQUENCE "%s" ,"%s"'%(seqname,pathstr))
        #self.write('MMEMORY:SAVE:SEQUENCE "{0}" ,"{1}"'.format(filename,pathstr))
        # the above command is overlapping, but we want a blocking command
        self.ask('*OPC?')    
